...
const fetchFiles = async (): Promise<string[]> => {
	const response = await fetch(`${import.meta.env.VITE_APP_FLASK_URL}/api/files`)
	if (!response.ok) {
	...
	return (
		<Card>
			...
			<table className="min-w-full table-auto whitespace-nowrap">
				<thead>
					<tr>
						<th scope="col" className="border-b-2 px-3 py-2 text-left tracking-wider">
							File Name
						</th>
						<th scope="col" className="border-b-2 px-3 py-2 tracking-wider">
							Actions
						</th>
					</tr>
				</thead>
				<tbody>
					{files?.map((file) => (
						<tr key={file}>
							<td className="overflow-hidden border-b border-neutral-300 px-3 py-2">
