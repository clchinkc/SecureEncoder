// src/components/TextProcessor.jsx
...
const TextProcessor: FC<TextProcessorProps> = ({
	encodingButtonRef,
	decodingButtonRef
}) => {
	const {
	...
	const saveTextMutation = useMutation<{ message: string }, Error, string>({
		mutationFn: async (newText: string) => {
			const response = await fetch(
				`${import.meta.env.VITE_APP_FLASK_URL}/api/save_text`,
				{
					method: 'PATCH',
					headers: {
	...
	>({
		mutationFn: async (data: {
			...
			const response = await fetch(
				`${import.meta.env.VITE_APP_FLASK_URL}/api/process_text`,
				{
					method: 'POST',
					headers: {
						'Content-Type': 'application/json'
