...
def process_text():
    ...
    operations = {
        'encode': {
            ...
            'ascii': encode_ascii,
            'url': encode_url,
            'aes': lambda x: aes_encrypt(x, ensure_aes_key(os.path.join(current_app.config['UPLOAD_FOLDER'], "aes_key.pem"))),
            'rsa': lambda x: rsa_encrypt(x, ensure_rsa_public_key(os.path.join(current_app.config['UPLOAD_FOLDER'], "rsa_public_key.pem"))),
            'md5': md5_encode,
        ...
        'decode': {
            ...
            'ascii': decode_ascii,
            'url': decode_url,
            'aes': lambda x: aes_decrypt(x, ensure_aes_key(os.path.join(current_app.config['UPLOAD_FOLDER'], "aes_key.pem"))),
            'rsa': lambda x: rsa_decrypt(x, ensure_rsa_private_key(os.path.join(current_app.config['UPLOAD_FOLDER'], "rsa_private_key.pem"))),
            'md5': md5_decode,
